...
# --- KEYBOARD SYSTEM (STRICT LOCK) ---
def get_main_keyboard(chat_id):
    user = users_col.find_one({"id": chat_id})
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    
    # Force Profile Creation: If no photo or gender, they see NOTHING but Create Profile
...
def start_reg(message):
    ...
    msg = bot.send_message(message.chat.id, "Welcome! To begin, please enter your **Full Name**:")
...
def reg_name(message):
    ...
    kb.add("Male 🧑", "Female 👩")
    msg = bot.send_message(message.chat.id, f"Hello {name}, select your Gender:", reply_markup=kb)
...
def reg_loc(message, name, gender, age):
    loc = message.text
    msg = bot.send_message(message.chat.id, "📸 Send your Profile Photo (STRICTLY Photos only! No Video/GIF):")
    bot.register_next_step_handler(msg, reg_photo, name, gender, age, loc)

def reg_photo(message, name, gender, age, loc):
    if message.content_type != 'photo':
        msg = bot.send_message(message.chat.id, "🚫 REJECTED! You must send a PHOTO. Try again:")
        bot.register_next_step_handler(msg, reg_photo, name, gender, age, loc)
        return
    ...
    bot.send_message(message.chat.id, "✅ Profile Verified! You can now find matches.", reply_markup=get_main_keyboard(message.chat.id))
...
def send_match(chat_id):
    ...
    if not is_p and count >= 10:
        return bot.send_message(chat_id, "🚫 Free Limit Reached (10/10). Please Buy Premium to continue!")
    ...
    if match:
        ...
        kb.row(types.InlineKeyboardButton("👍", callback_data="reaction"), 
               types.InlineKeyboardButton("❤️", callback_data="reaction"), 
               types.InlineKeyboardButton("😍", callback_data="reaction"))
        ...
        if not is_p: cap += f"\n\n📊 Usage: {count}/10 used. ({10-count} left)"
        
        bot.send_photo(chat_id, target['photo'], caption=cap, reply_markup=kb)
    else:
        bot.send_message(chat_id, "No more new profiles. Check back later!")

@bot.message_handler(commands=['start'])
def start_cmd(message):
    if is_banned(message.chat.id):
        return bot.send_message(message.chat.id, "🚫 Access denied. You are banned.")
    if not is_joined(message.chat.id):
        kb = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("Join Channel 🔒", url=f"https://t.me/{CHANNEL_USERNAME[1:]}"), types.InlineKeyboardButton("Verified ✅", callback_data="check_v"))
        return bot.send_message(message.chat.id, "Welcome! To use this Dating App, you MUST join our channel first.", reply_markup=kb)
...
def handle_all_text(message):
    ...
    if not user or "photo" not in user:
        ...
        return bot.send_message(message.chat.id, "🔒 Lock: You must create a profile to access the menu!", reply_markup=get_main_keyboard(message.chat.id))

    if message.text == "🔍 Find Matches":
        send_match(message.chat.id)
    elif message.text == "👤 My Profile":
        bot.send_photo(message.chat.id, user['photo'], caption=f"Your Profile Details:\nName: {user['name']}\nGender: {user['gender']}\nStatus: {'Premium' if check_premium(message.chat.id) else 'Free'}")
        ...
        bot.send_message(message.chat.id, "Click below to talk to Support:", reply_markup=kb)
...
def callbacks(call):
    if call.data == "check_v":
        ...
        if res['ok']:
            ...
            bot.send_message(call.message.chat.id, f"Please complete your payment of ${price}:", reply_markup=kb)
